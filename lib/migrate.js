...
// Chromium cache directories: huge (hundreds of MB), regenerate automatically, and
// carry nothing of the user's. Copying them would only slow the first Caryl boot down.
const SKIP_DIRS = new Set([
  'Cache', 'Code Cache', 'GPUCache', 'DawnGraphiteCache', 'DawnWebGPUCache',
  'ShaderCache', 'blob_storage', 'Crashpad', 'Crash Reports'
]);
...
function migrateUserData(newDir, legacyDirs) {
  try {
    ...
    fs.cpSync(from, newDir, {
      recursive: true, force: false, errorOnExist: false,
      filter: (src) => !SKIP_DIRS.has(path.basename(src)) // prune cache dirs at any depth
    });
